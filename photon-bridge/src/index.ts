...
api.post("/send", async (req, res) => {
  ...
  try {
    // Spectrum resolves a direct iMessage space from the recipient handle, then sends natively.
    const message = await spectrum.send(to, text);
    return res.json({mode:"live", message_id: message?.id ?? null});
...
async function receiveMessages() {
  for await (const [space, message] of spectrum.messages) {
    ...
    const sender = String(message.sender?.id ?? message.sender?.address ?? "");
