...
function render(status) {
  ...
  if(ringing){set('Wingman 正在呼叫你','点击接听后播放你的录音。');ringNow();}
...
async function poll(){if(!base)return;try{render((await api('/api/v1/demo/call')).status)}catch(e){$('hint').textContent=e.message}}
